...
def sync_action_for_transaction(transaction_id: str, action_type: str, actor: str = "cfo") -> None:
    """
    Synchronizes the state machine SQLite database when an action is triggered via /apply-action.
    Ensures GET /actions and SQLite tables reflect the executed action immediately.
    """
    conn = get_db_connection()
    now_str = datetime.utcnow().isoformat() + "Z"
    try:
        with conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT action_id, status FROM recommendations WHERE transaction_id = ? ORDER BY created_at DESC",
                (transaction_id,)
            )
            row = cursor.fetchone()
            if row:
                ...
                conn.execute(
                    """
                    INSERT INTO executions (execution_id, action_id, status, timestamp)
                    VALUES (?, ?, 'EXECUTED', ?)
                    """,
                    (exec_id, action_id, now_str)
                )
                conn.execute(
                    """
                    INSERT INTO audit_logs (event_type, action_id, transaction_id, timestamp, actor, old_state, new_state)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                    """,
                    ("EXECUTION_COMPLETED", action_id, transaction_id, now_str, actor, row["status"], "EXECUTED")
                )
            else:
                # Create executed recommendation
                action_id = f"act_{uuid.uuid4().hex[:8]}"
                conn.execute(
                    """
                    INSERT INTO recommendations 
                    (action_id, transaction_id, action_type, priority, risk_score, confidence, reason, 
                     reason_codes_json, warnings_json, amount_base, status, created_at, updated_at)
                    VALUES (?, ?, ?, 'HIGH', 80, 95, ?, '[]', '[]', 0.0, 'EXECUTED', ?, ?)
                    """,
                    (action_id, transaction_id, action_type, f"Direct action trigger {action_type}", now_str, now_str)
                )
                conn.execute(
                    """
                    INSERT INTO audit_logs (event_type, action_id, transaction_id, timestamp, actor, new_state)
                    VALUES (?, ?, ?, ?, ?, ?)
                    """,
                    ("EXECUTION_COMPLETED", action_id, transaction_id, now_str, actor, "EXECUTED")
                )
    except Exception as e:
        logger.warning("Failed to sync action to state machine DB: %s", e)
    finally:
        conn.close()
