...
def get_model_diagnostics(
    ...
    try:
        ...
        # Calculate flat historical daily volatility and EWMA daily volatility (decay=0.94)
        flat_daily_vol = {ccy: round(float(np.sqrt(cov_matrix[i, i])), 6) for i, ccy in enumerate(ccy_list)}
        ewma_daily_vol = {}
        decay = 0.94
        n_obs = len(return_matrix)
        weights = (1.0 - decay) * (decay ** np.arange(n_obs - 1, -1, -1))
        weights /= weights.sum()
        for i, ccy in enumerate(ccy_list):
            ret_series = return_matrix[:, i]
            ewma_var = np.sum(weights * (ret_series ** 2))
            ewma_daily_vol[ccy] = round(float(np.sqrt(ewma_var)), 6)
        ...
        return {
            ...
            "flat_historical_daily_volatility": flat_daily_vol,
            "ewma_daily_volatility": ewma_daily_vol,
